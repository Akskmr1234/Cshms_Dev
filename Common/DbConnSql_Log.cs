...
class DbConnSql_Log
{
    ...
    public DbConnSql_Log()
    {
        try
        {
            strConString = System.Configuration.ConfigurationSettings.AppSettings.Get("LogSqlConString");
            strDBType = "S";
            strErrorDesc = "";
    ...
    public DbConnSql_Log(String ConSring)
    {
        try
        {
    ...
    public DbConnSql_Log(SqlConnection Con)
    {
        Conn = Con;
    }
    public DbConnSql_Log(Object Con)
    {
    ...
    public void InsertDataTableWithSql(DataTable dt,String vPrimaryKey, String vFilterCond)
    {
        ...
        for (int intRow = 0; intRow < dt.Rows.Count; intRow++)
        {
            ...
            foreach (DataColumn column in dt.Columns)
            {
                if (vPrimaryKey != column.ColumnName)
                {
                    if (dt.Columns[intCols].DataType.Name.ToLower().Contains("int") ||
                            ...
                            dt.Columns[intCols].DataType.Name.ToLower().Contains("decimal"))
                    {
                        ...
                        strRet += mComFuncs.FormatDBServDateWithTime(dt.Rows[intRow][intCols].ToString()) + ", ";
                    else
                        strRet += "'" + dt.Rows[intRow][intCols].ToString() + "', ";
                }
                intCols++;
